...
async def node_port_svc(task: TrainingTask = Depends(get_api_task()),
                          ...
                          dist_port: int = 22, user: User = Depends(get_api_user_with_token(allowed_groups=['internal']))):
    assert rank >= 0
    try:
...
async def delete_node_port_svc(task: TrainingTask = Depends(get_api_task()),
                               ...
                               user: User = Depends(get_api_user_with_token(allowed_groups=['internal']))):
    assert rank >= 0
    try:
...
async def get_node_port_svc_list(user: User = Depends(get_api_user_with_token(allowed_groups=['internal']))):
    return {'success': 1, 'result': await user.nodeport.async_get()}


async def task_ssh_ip(pod_name: str, user: User = Depends(get_api_user_with_token(allowed_groups=['internal']))):
    ip = (await run_cmd_aio(f"kubectl -n {CONF.launcher.task_namespace} get pod {pod_name} -o wide | grep -v NAME | awk '{{print $6}}'"))[0].decode().replace("\n", "")
    return {
            'success': 1,
            'ip': ip
        }
